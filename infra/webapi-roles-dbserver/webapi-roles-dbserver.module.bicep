...
param dbserver_outputs_name string

param dbserver_outputs_sqlserveradminname string

param principalId string

param principalName string

resource dbserver 'Microsoft.Sql/servers@2023-08-01' existing = {
  name: dbserver_outputs_name
}

resource sqlServerAdmin 'Microsoft.ManagedIdentity/userAssignedIdentities@2024-11-30' existing = {
  name: dbserver_outputs_sqlserveradminname
}
...
resource script_dbserver_mydemo2Db 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('dbserver', principalName, 'mydemo2Db', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
